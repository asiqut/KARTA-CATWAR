...
function normalize(p:Point[]):Point[]{const d=p.reduce<Point[]>((a,v)=>!a.length||!same(a[a.length-1],v)?[...a,v]:a,[]),r:Point[]=[];d.forEach((v,i)=>{if(i>0&&i<d.length-1){const a=d[i-1],b=d[i+1],sv=Math.abs(a.x-v.x)<5&&Math.abs(v.x-b.x)<5,sh=Math.abs(a.y-v.y)<5&&Math.abs(v.y-b.y)<5;if(sv||sh)return}r.push(v)});return r}
...
export function App(){
 ...
 function moveLocation(e:ReactPointerEvent<SVGElement>){
  ...
  setConnectors(v=>v.map(c=>{if(c.from.locationId!==d.id&&c.to.locationId!==d.id)return c;const fl=map.get(c.from.locationId),tl=map.get(c.to.locationId);if(!fl||!tl)return c;const moved={...fl,x:d.id===fl.id?nx:fl.x,y:d.id===fl.id?ny:fl.y},toL=d.id===tl.id?{...tl,x:nx,y:ny}:tl,dx=nx-d.x,dy=ny-d.y;const fromPoint=c.fromPoint&&d.id===c.from.locationId?{x:c.fromPoint.x+dx,y:c.fromPoint.y+dy}:c.fromPoint,toPoint=c.toPoint&&d.id===c.to.locationId?{x:c.toPoint.x+dx,y:c.toPoint.y+dy}:c.toPoint;if(c.points&&c.points.length>=2){const next=c.points.map(p=>({...p}));if(d.id===c.from.locationId){if(next[0])next[0]={x:next[0].x+dx,y:next[0].y+dy};if(next[1])next[1]={x:next[1].x+dx,y:next[1].y+dy}}if(d.id===c.to.locationId){const j=next.length-1;if(next[j])next[j]={x:next[j].x+dx,y:next[j].y+dy};if(next[j-1])next[j-1]={x:next[j-1].x+dx,y:next[j-1].y+dy}}return{...c,fromPoint,toPoint,points:normalize(next)}}const ng=geometryWithLocations({...c,fromPoint,toPoint},moved,toL);if(!ng)return c;if(c.oneWay){const best=bestOneWay(moved,ng.ft!,toL,locations);return{...c,fromSide:best.fs,toSide:best.ts,fromPoint:best.pts[0],toPoint:best.b}}const best=bestNormal(moved,ng.ft!,toL,ng.tt!,locations);return{...c,fromSide:best.fs,toSide:best.ts,fromPoint:best.pts[0],toPoint:best.pts[best.pts.length-1]}}));
 }
